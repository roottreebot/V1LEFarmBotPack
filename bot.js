...
async function sendOrEdit(id,text,opt={}){ 
    ...
    try{ if(mid){ await bot.editMessageText(text,{chat_id:id,message_id:mid,...opt}); return; } }catch{}
...
bot.on('message', msg => {
    ...
    // Skip if not in order input step
...
🌿 ${s.product}
⚖️ ${grams}g
💲 $${cash}`,{parse_mode:'Markdown', reply_markup:{inline_keyboard:[
...
bot.onText(/\/profile/, msg => {
    const id=msg.chat.id; if(banGuard(id)||isRateLimited(id)) return;
    ensureUser(id,msg.from.username);
...
bot.onText(/\/top/, msg => {
    const id=msg.chat.id; if(banGuard(id)||isRateLimited(id)) return;
    checkWeeklyReset();
    const top=Object.entries(users).filter(([,u])=>!u.banned).sort((a,b)=>b[1].weeklyXp-b[1].weeklyXp).slice(0,10);
    let txt=`${HEADER}\n🏆 *Weekly Top Farmers*\n\n`;
